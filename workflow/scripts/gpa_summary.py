...
def get_options():
    description = 'Make a summary of gene presence/absence tests results'
    parser = argparse.ArgumentParser(description=description)

    parser.add_argument('gpa',
                        help='GPA test results table')
...
if __name__ == "__main__":
    ...
    if options.pangenome_genes is not None:
        ...
        roary = roary[sorted(set(options.reference))]
    ...
    a = pd.read_csv(options.gpa, sep='\t', index_col=0)
    # check empty
    if a.shape[0] == 0:
        sys.exit(0)
    # add pangenome info
